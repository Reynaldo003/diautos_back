...
from .serializers import (
    DetalleVentasPostVentaDetailSerializer,
...
def construir_resumen_servicios(historial, kilometraje_actual=None):
    ...
    for item in historial:
        ...
        historial_mapeado.append((item, texto_normalizado))

    for servicio in SERVICIOS_COMERCIALES:
        coincidencias = []

        for item, texto in historial_mapeado:
...
class OrdenServicioVentaDiautosViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    ordering_fields = [
        "id", "fecha_venta", "fecha_os", "ano_modelo",
        "importe_factura", "costo_os", "dias_os_a_actual",
        "meses_actual_a_venta", "prioridad_prospeccion",
    ]
    ...
    def get_queryset(self):
        ...
        nombre_cte = obtener_query_param(query_params, "nombre_cte", "nombre")
        numero_serie = obtener_query_param(query_params, "numero_serie", "vin")
        celular = obtener_query_param(query_params, "celular")
        email = obtener_query_param(query_params, "email")
        ...
        prioridad_prospeccion = obtener_query_param(
            query_params, "prioridad_prospeccion", "prioridadProspeccion"
        )

        dias_operador = obtener_query_param(query_params, "dias_operador", "operadorDiasIngreso")
        dias_valor = obtener_query_param(query_params, "dias_valor", "valorDiasIngreso")
        meses_venta_operador = obtener_query_param(query_params, "meses_venta_operador", "operadorMesesVenta")
        meses_venta_valor = obtener_query_param(query_params, "meses_venta_valor", "valorMesesVenta")
        ...
        if q:
            # Reducido a los campos más buscados — la tabla es externa (managed=False)
            # y no podemos agregar índices, así que menos icontains = más rápido
            queryset = queryset.filter(
                Q(nombre_cte__icontains=q)
                | Q(celular__icontains=q)
                | Q(numero_serie__icontains=q)  # tiene db_index
                | Q(vendedor__icontains=q)
                | Q(folio_factura__icontains=q)  # tiene db_index
                | Q(id_os__icontains=q)           # tiene db_index
        ...
        if prioridad_prospeccion:
            queryset = queryset.filter(prioridad_prospeccion__iexact=prioridad_prospeccion)

        if dias_gt not in (None, ""):
            queryset = aplicar_filtro_numerico(queryset, "dias_os_a_actual", "mayor", dias_gt)
        elif dias_lt not in (None, ""):
            queryset = aplicar_filtro_numerico(queryset, "dias_os_a_actual", "menor", dias_lt)
        elif dias_exact not in (None, ""):
            queryset = aplicar_filtro_numerico(queryset, "dias_os_a_actual", "igual", dias_exact)
        else:
            queryset = aplicar_filtro_numerico(queryset, "dias_os_a_actual", dias_operador, dias_valor)

        if meses_gt not in (None, ""):
            queryset = aplicar_filtro_numerico(queryset, "meses_actual_a_venta", "mayor", meses_gt)
        elif meses_lt not in (None, ""):
            queryset = aplicar_filtro_numerico(queryset, "meses_actual_a_venta", "menor", meses_lt)
        elif meses_exact not in (None, ""):
            queryset = aplicar_filtro_numerico(queryset, "meses_actual_a_venta", "igual", meses_exact)
        else:
            queryset = aplicar_filtro_numerico(queryset, "meses_actual_a_venta", meses_venta_operador, meses_venta_valor)
    ...
    def estadisticas(self, request):
        ...
        # Una sola query en lugar de 3 count() separados
        conteos = queryset.aggregate(
            total=Count("id"),
            activos=Count(Case(
                When(estado_cliente__iexact="ACTIVO", then=1),
                output_field=IntegerField(),
            )),
            inactivos=Count(Case(
                When(estado_cliente__iexact="INACTIVO", then=1),
                output_field=IntegerField(),
            )),
        )
    ...
    def detalle_comercial(self, request, pk=None):
        ...
        # Guard: si no hay VIN evitamos queries innecesarias
        if vin:
            ...
            comentarios_os = obtener_comentarios_os_por_vin(vin)
        else:
            historial = []
            comentarios_os = RetencionComentario.objects.none()

        ultimo_registro = historial[0] if historial else None
        fecha_ultimo_historial = (
            obtener_fecha_postventa(ultimo_registro) if ultimo_registro else None
        )
        dias_desde_ultimo_historial = (
            (date.today() - fecha_ultimo_historial).days
        ...
        return Response(
            {
                ...
                "resumen": {
                    "vin": vin,
                    "total_ordenes_historial": len(historial),
                ...
                "trabajos_recientes": construir_trabajos_recientes(historial),
                ...
                "historial": DetalleVentasPostVentaDetailSerializer(
                    historial,
                    many=True,
                ).data,
            }
