...
def obtener_ventas_base(fecha_venta_desde=None, fecha_venta_hasta=None):
    ...
    queryset = (
        ...
        .only(
            "id", "numero_serie", "nombre_cte", "telefono", "celular",
            "email", "marca_vehiculo", "version", "ano_modelo",
            "fecha_venta", "folio_factura", "vendedor", "fecha_os",
            "id_os", "asesor", "estado_cliente", "dias_os_a_actual",
            "meses_actual_a_venta", "franja_retencion",
            "prioridad_prospeccion", "kilometraje",
        )
        .order_by("numero_serie", "fecha_venta", "id")
    )
    ...
    for venta in queryset:
        ...
        existente = ventas_unicas_por_vin.get(vin)
        if existente is None:
            ventas_unicas_por_vin[vin] = venta
            continue

        fecha_nueva = venta.fecha_venta or date.min
        fecha_actual = existente.fecha_venta or date.min
        if (fecha_nueva, venta.id) > (fecha_actual, existente.id):
            ventas_unicas_por_vin[vin] = venta
...
class CarteraClienteViewSet(viewsets.ModelViewSet):
    ...
    def resumen(self, request):
        ...
        # Una sola query en lugar de 4 count() separados
