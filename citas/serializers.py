...
class CitaSerializer(BaseConClienteInputMixin):
    """Maneja el guardado (POST/PATCH). Al usar __all__, ya procesa 'estado_gestion' automáticamente."""
    class Meta:
        model = Cita
        fields = "__all__"  # ← Esto ya incluye 'detalle_gestion' automáticamente por el modelo


class CitaListSerializer(serializers.ModelSerializer):
    """Serializer ligero solo para la vista de tabla/lista."""
    # Nota: Tu código de React busca los datos como 'cita.cliente.nombre', por eso mapeamos el objeto completo
    cliente = ClienteComercialSerializer(read_only=True)
    ...
    class Meta:
        ...
        fields = [
            ...
            "asesor_piso",
            "estado_gestion",
            "detalle_gestion",   # ← NUEVO CAMPO (agregar esto)
            "comentarios",
        ]